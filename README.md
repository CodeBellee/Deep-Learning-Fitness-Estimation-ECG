...
1. Create .csv files with create create_fitness_params.py. Special case: blood pressure .csv files were creates with BP_mean.py as teh average of 2 measurements was calculated.
2. #.csv file was created with sex, age (from instace 2), BMI (from instance 2) from participants with a resting ECG data file from insatnce 2
3. PCLR embeddings were extracted and saved with create_pclr_emb.py


4. pclr_plus_different_models.py creates a 'Full Model' (Input: Age, Sex, BMI, 320 ECG embeddings) with the Machine Learning techniques: Lasso, ElasicNet, XGBoost, MLP
...
5. basic_own_model.py creates a 'Basic Model' (Input: Age, Sex, BMI, 320 ECG embeddings) with the machine learning techniques: Lasso, ElasicNet, XGBoost, MLP
This file was used to generate the Basic Model for all machine learning techniques for the VO2max evaluation.

7. For efficiency, the basic model was not run manually for every fitness parameter.
