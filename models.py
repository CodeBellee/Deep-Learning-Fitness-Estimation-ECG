...
# Choose Strategy: 'BASIC', 'ECG_ONLY', or 'FULL'
MODEL_STRATEGY = 'FULL'

# Choose Model: 'LASSO', 'XGBOOST', 'MLP', or 'ELASTICNET'
MODEL_USED = 'LASSO'

BASE_DIR = "/YOUR/BASE/PATH/DIRECTORY"
LABELS_BASE_DIR ="/YOUR/BASE/PATH/DIRECTORY"
OUTPUT_DIR = "/YOUR/PATH/DIRECTORY"
MODEL_SAVE_DIR = "/YOUR/PATH/DIRECTORY"

AGE2_SEX_BMI2_PATH = "/YOUR/BASE/PATH/DIRECTORY"
EMBEDDINGS_PATH = "/YOUR/BASE/PATH/DIRECTORY"
...
def get_common_features(data: pd.DataFrame, target_col: str):
    """Identifies available features based on the selected MODEL_STRATEGY."""
    
    clinical_features = ['age', 'sex', 'bmi']
    pclr_features = [f'pclr_output_{i}' for i in range(320)]
    
    if MODEL_STRATEGY == 'BASIC':
        all_potential_features = clinical_features
    elif MODEL_STRATEGY == 'ECG_ONLY':
        all_potential_features = pclr_features
    elif MODEL_STRATEGY == 'FULL':
        all_potential_features = clinical_features + pclr_features
    else:
        raise ValueError("Invalid MODEL_STRATEGY selected.")
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f"ERROR: Clinical data not found. {e}")
        exit()

    # Conditionally load heavy ECG embeddings
    if MODEL_STRATEGY in ['FULL', 'ECG_ONLY']:
        try:
            pclr_emb = pd.read_csv(EMBEDDINGS_PATH)
        except FileNotFoundError as e:
            print(f"ERROR: Embeddings not found. {e}")
            exit()
    ...
    for target_label in TARGETS_TO_RUN:
        ...
        # Save Model & Scaler
        # Save Model and Scaler dynamically
        model_filename = os.path.join(MODEL_SAVE_DIR, f"{MODEL_STRATEGY.lower()}_{target_label.lower()}_{MODEL_USED.lower()}_model.pkl")
        scaler_filename = os.path.join(MODEL_SAVE_DIR, f"{MODEL_STRATEGY.lower()}_{target_label.lower()}_{MODEL_USED.lower()}_scaler.pkl")
        ...
        out_file = os.path.join(OUTPUT_DIR, f"{MODEL_STRATEGY.lower()}_{target_label.lower()}_{MODEL_USED.lower()}.csv")
